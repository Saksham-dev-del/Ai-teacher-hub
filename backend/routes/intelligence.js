...
const {explainConcept,simplifyExplanation}=require('../services/conceptExplainer');
const {generateAnalogies,explainWithTheme,THEME_LENSES}=require('../services/analogyEngine');
const {generateStoryboard}=require('../services/storyboardGenerator');
const {generateSimulation}=require('../services/liveSimulation');
const {generateRevisionPack}=require('../services/revisionEngine');
const {suggestRelatedTopics}=require('../services/knowledgeGraph');
...
router.get('/analogy-themes', (req,res)=>{res.json({themes:THEME_LENSES});});
router.post('/analogies', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await generateAnalogies(topic,cleanText(req.body?.conceptType,80));res.json(data);}catch(err){next(err)}});
router.post('/analogy-explain', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);const themeKey=cleanText(req.body?.theme,40);if(!topic||!themeKey)return res.status(400).json({error:'Topic and theme are required.'});const data=await explainWithTheme(topic,themeKey,cleanText(req.body?.conceptType,80));res.json(data);}catch(err){next(err)}});
router.post('/storyboard', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await generateStoryboard(topic,cleanText(req.body?.conceptType,80));const artifact=await PhaseArtifact.create({owner:req.user._id,phase:11,category:'storyboard',action:'generate',title:data.title,topic,inputs:{topic},output:data,generationMode:data.generationMode||'fallback'});res.status(201).json({...data,artifact});}catch(err){next(err)}});
router.post('/simplify-explanation', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await simplifyExplanation(topic,cleanText(req.body?.conceptType,80),cleanText(req.body?.previousExplanation,900));res.json(data);}catch(err){next(err)}});
router.post('/simulation', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await generateSimulation(topic,cleanText(req.body?.conceptType,80));const artifact=await PhaseArtifact.create({owner:req.user._id,phase:11,category:'simulation',action:'generate',title:data.title,topic,inputs:{topic},output:data,generationMode:data.generationMode||'fallback'});res.status(201).json({...data,artifact});}catch(err){next(err)}});
router.post('/revision-pack', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await generateRevisionPack(topic,cleanText(req.body?.conceptType,80));const artifact=await PhaseArtifact.create({owner:req.user._id,phase:11,category:'revision-pack',action:'generate',title:topic,topic,inputs:{topic},output:data,generationMode:data.generationMode||'fallback'});res.status(201).json({...data,artifact});}catch(err){next(err)}});
router.post('/related-topics', async(req,res,next)=>{try{const topic=cleanText(req.body?.topic,220);if(!topic)return res.status(400).json({error:'Enter a topic first.'});const data=await suggestRelatedTopics(topic,cleanText(req.body?.conceptType,80));res.json(data);}catch(err){next(err)}});
